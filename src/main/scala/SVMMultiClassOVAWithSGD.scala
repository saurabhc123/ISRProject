...
class SVMMultiClassOVAModel(classModels: Array[SVMModel])  {
   ...
   */
  def predict(testData: RDD[LabeledPoint]): RDD[Double] = {
   ...
   */
  def predict(testData: LabeledPoint): Double = predictPoint(testData, classModelsWithIndex)

  def predictPoint(testData: LabeledPoint, models: Array[(SVMModel, Int)]): Double =
    models
      .map { case (classModel, classNumber) => (classModel.predict(testData.features), classNumber)}
